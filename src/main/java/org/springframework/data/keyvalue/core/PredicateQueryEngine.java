 ...
 * {@link KeyValueAdapter}. Note uses a {@link SpelSortAccessor} for sorting.
 *
...
class PredicateQueryEngine extends QueryEngine<KeyValueAdapter, Predicate, Comparator<?>> {
	...
	public PredicateQueryEngine() {
		super(new PredicateCriteriaAccessor(), new SpelSortAccessor(PARSER));
	}
	...
	private static <S> List<S> filterMatchingRange(List<S> source, @Nullable Predicate criteria, long offset,
			int rows) {
